...
const DiscussionItem = ({
  ...
  const { createdDate, id, title, rating, user, discussionReplies, courseName } = disc;

  const { stateYear, stateMonth, stateDay, isToday } = useDate(createdDate);

  return (
    <ul className="list-none m-0 p-0 table table-fixed text-[1.2rem] text-color-primary w-full bg-[white] border-b-[1px]">
      <li className="table-item vs:w-[50%] lg:!w-[45rem] border-l-[1px]">
        {showTop && <p className="text-[0.9rem] text-black">{courseName}</p>}
        <Link to={`/discussion/${id}`}>
          <span className="duration-[0.4s] hover:text-[crimson]">{title}</span>
        </Link>
